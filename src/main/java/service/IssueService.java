...
//        return issue;
    ...
    private final IssueValidationService  issueValidationService = new IssueValidationService();

    public Issue createIssue(Issue issue) throws Exception {
        List<String> errors = issueValidationService.validateIssueForCreate(issue);
        if (!errors.isEmpty()) {
    ...
    public void updateIssue(Issue issue) throws Exception {
        ...
        List<String> errors = issueValidationService.validateIssueForUpdate(issue);
        if (!errors.isEmpty()) {
            throw new IllegalArgumentException("Validation failed: " + String.join(", ", errors));
        }
    ...
    public static class SQL {
        ...
        // Analytics queries
        public static final String COUNT_ISSUES_BY_PROJECT =
                "SELECT COUNT(*) FROM issues WHERE project_id = ?";

        public static final String COUNT_ISSUES_BY_STATUS =
                "SELECT COUNT(*) FROM issues WHERE status_id = ?";

        public static final String SELECT_RECENT_ISSUES =
                "SELECT * FROM issues ORDER BY updated_at DESC LIMIT ?";
